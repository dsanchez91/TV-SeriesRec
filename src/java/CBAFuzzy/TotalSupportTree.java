...
T-tree data storage structures used to hold the total support counts for large itemsets.
@author Frans Coenen
@version 3 July 2003 */
...
public class TotalSupportTree extends AssocRuleMining {
    ...
    public TotalSupportTree(String[][] trainingData, int numClasses, double minSup, double minConf) {
    ...
    public void createTotalSupportTree() {	
		...
		// Create Top level of T-tree (First pass of dataset). Defined in TotlaSupportTree class.
		createTtreeTopLevel();
    ...
    protected void createTtreeTopLevel2() {
        // Loop through data set record by record and add support for each 1 itemset
		for (int index1=0;index1<dataArray.length;index1++) {
		    ...
		    if (dataArray[index1] != null) {
	   	        for (int index2=0;index2<dataArray[index1].length;index2++) {
				    startTtreeRef[dataArray[index1][index2]].support = startTtreeRef[dataArray[index1][index2]].support + Float.parseFloat(supports.get(index1).toString()); 
				    numUpdates++;
			    }
			}
		}
	...
	// Loop through data set record by record
        for (int index=0;index<dataArray.length;index++) {
		    ...
		    if (dataArray[index] != null) 
	        	addSupportToTtreeFindLevel(startTtreeRef,level,dataArray[index].length,dataArray[index],Float.parseFloat(supports.get(index).toString()));
	    }
	} 
    ...
    @param linkRef the reference to the current sub-branch of T-tree (start at top of tree)
    @param level the level marker, set to the required level at the start and then decremented by 1 on each recursion. 
    ...
    protected void addSupportToTtreeFindLevel(TtreeNode[] linkRef, int level, int endIndex, short[] itemSet, float support) {
		// At right level;		
		if (level == 1) {
		    ...
		    for (int index1=0;index1 < endIndex;index1++) {
				...
				if (linkRef[itemSet[index1]] != null) {
				    linkRef[itemSet[index1]].support = linkRef[itemSet[index1]].support + support; 
				    numUpdates++;
				}
			}
	    } else {	// At wrong level
		    ...
		    for (int index=level-1;index<endIndex;index++) {		
				...
				if (linkRef[itemSet[index]] != null) {
				    if (linkRef[itemSet[index]].childRef != null) 
				    	 addSupportToTtreeFindLevel(linkRef[itemSet[index]].childRef,level-1,index,itemSet,support);
				}
			}
		}
	}
    ...
    protected void generateLevel2() {
		...
		// level from index 1 as there will be no proceeding attributes, remember index 0 is unused.	
		...
		if (level == 1) {
		    ...
		    for (int index=2;index<localSize;index++) {
		    	...
		    	if (linkRef[index] != null)
		    		generateNextLevel(linkRef,index,realloc2(itemSet,(short) index));
			}
		} else { // Wrong level		
		    for (int index=level;index<localSize;index++) {
		        // If supported T-tree node
		        if (linkRef[index]!=null && linkRef[index].childRef!=null) 
		               generateLevelN(linkRef[index].childRef,level-1,realloc2(itemSet,(short) index));
			}
	    }
	}
    ...
    contained in the current sub-branch of the T-tree, {D,B,A} and {C,B,A} are not.
    </OL> 
    ...
    */	
    ...
    protected float getSupportForItemSetInTtree(short[] itemSet) {
		int endInd = itemSet.length-1;

    	// Last element of itemset in Ttree (Note: Ttree itemsets stored in reverse)
	  	if (startTtreeRef[itemSet[endInd]] != null) {
		    // If "current index" is 0, then this is the last element (i.e the input is a 1 itemset)  and therefore item set found
		    if (endInd == 0) 
		    	return(startTtreeRef[itemSet[0]].support);
		    else { // Otherwise continue down branch
		    	TtreeNode[] tempRef = startTtreeRef[itemSet[endInd]].childRef;
		        if (tempRef != null) 
		        	return(getSupForIsetInTtree2(itemSet,endInd-1,tempRef));
				else // No further branch therefore rerurn 0 
					return(0);
			}
		} else  // Item set not in Ttree thererfore return 0 
	    	return(0);
	}
    ...
    private float getSupForIsetInTtree2(short[] itemSet, int index, TtreeNode[] linkRef) {  
    	...
    	if (linkRef[itemSet[index]] != null) {
		    ...
		    if (index == 0)  // Otherwise continue provided there is a child branch to follow 
		    	return(linkRef[itemSet[0]].support);
		    else if(linkRef[itemSet[index]].childRef != null)
		    	return(getSupForIsetInTtree2(itemSet,index-1,linkRef[itemSet[index]].childRef));
		    else
		    	return(0);
	    } else // Item set not in Ttree therefore return 0
	    	return(0);    
    }
    ...
    @param support the support for the large itemset from which the AR is generated.
		...
		for (int index=1; index <= numOneItemSets; index++) {
		    if (startTtreeRef[index] !=null) {
		        if (startTtreeRef[index].support >= minSupport) {
		            System.out.println("[" + number + "] {" + index + "} = " + startTtreeRef[index].support);
		            number = outputFrequentSets(number+1, new Integer(index).toString(), index,startTtreeRef[index].childRef);
			    }
			}
		}
		// End	
    ...
    @param linkRef the reference to the current array level in the T-tree. 
    @return the incremented (possibly) number the number of frequent sets so far. */
    ...
    public void outputNumFreqSets() {
		...
		else	// Otherwise count and output
    ...
    public void outputNumFreqSetsPerBranch() { 
		...
		for (int index=1; index <= numOneItemSets; index++) {
		    if (startTtreeRef[index] !=null) {
		        System.out.println("(" + index + ")" + countNumFreqSets(index,startTtreeRef[index].childRef,1));
			}	   
	    }
	}
